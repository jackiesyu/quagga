...
setsockopt_so_recvbuf (int sock, int size)
{
  ...
  if ( (ret = setsockopt (sock, SOL_SOCKET, SO_RCVBUF, (char *)
                          &size, sizeof (int))) < 0)
    zlog_err ("DR7450:fd %d: can't setsockopt SO_RCVBUF to %d: %s",
...
setsockopt_so_sendbuf (const int sock, int size)
{
  ...
  if (ret < 0)
    zlog_err ("DR7451:fd %d: can't setsockopt SO_SNDBUF to %d: %s",
      sock, size, safe_strerror (errno));
...
getsockopt_so_sendbuf (const int sock)
{
  ...
  if (ret < 0)
  {
    zlog_err ("DR7452:fd %d: can't getsockopt SO_SNDBUF: %d (%s)",
...
setsockopt_ipv6_pktinfo (int sock, int val)
{
  ...
  if (ret < 0)
    zlog_warn ("DR4400:can't setsockopt IPV6_RECVPKTINFO : %s", safe_strerror (errno));
#else	/*RFC2292*/
  ret = setsockopt(sock, IPPROTO_IPV6, IPV6_PKTINFO, &val, sizeof(val));
  if (ret < 0)
    zlog_warn ("DR4401:can't setsockopt IPV6_PKTINFO : %s", safe_strerror (errno));
#endif /* INIA_IPV6 */
  return ret;
}
...
#endif /* GNU_LINUX */
  if (ret < 0)
    zlog_warn ("DR4402:can't setsockopt IPV6_CHECKSUM");
  return ret;
}
...
setsockopt_ipv6_multicast_hops (int sock, int val)
{
  ...
  if (ret < 0)
    zlog_warn ("DR4403:can't setsockopt IPV6_MULTICAST_HOPS");
  return ret;
}
...
setsockopt_ipv6_unicast_hops (int sock, int val)
{
  ...
  if (ret < 0)
    zlog_warn ("DR4404:can't setsockopt IPV6_UNICAST_HOPS");
  return ret;
}
...
setsockopt_ipv6_hoplimit (int sock, int val)
{
  ...
  if (ret < 0)
    zlog_warn ("DR4405:can't setsockopt IPV6_RECVHOPLIMIT");
#else	/*RFC2292*/
  ret = setsockopt (sock, IPPROTO_IPV6, IPV6_HOPLIMIT, &val, sizeof(val));
  if (ret < 0)
    zlog_warn ("DR4406:can't setsockopt IPV6_HOPLIMIT");
#endif
  return ret;
}
...
setsockopt_ipv6_multicast_loop (int sock, int val)
{
  ...
  ret = setsockopt (sock, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, &val,
		    sizeof (val));
  if (ret < 0)
    zlog_warn ("DR4407:can't setsockopt IPV6_MULTICAST_LOOP");
  return ret;
}
...
setsockopt_ipv6_tclass(int sock, int tclass)
{
  ...
  if (ret < 0)
    zlog_warn ("DR4408:Can't set IPV6_TCLASS option for fd %d to %#x: %s",
  ...
  if ((ret < 0) && (optname == IP_ADD_MEMBERSHIP) && (errno == EADDRINUSE))
    {
      ...
      zlog_info("DR1100:setsockopt_ipv4_multicast attempting to drop and "
  ...
  if ((ret < 0) && (optname == IP_ADD_MEMBERSHIP) && (errno == EADDRINUSE))
    {
      /* see above: handle possible problem when interface comes back up */
      char buf[1][INET_ADDRSTRLEN];
      zlog_info("DR1101:setsockopt_ipv4_multicast attempting to drop and "
                "re-add (fd %d, mcast %s, ifindex %u)",
                sock,
...
setsockopt_ipv4_ifindex (int sock, int val)
{
  ...
  if ((ret = setsockopt (sock, IPPROTO_IP, IP_PKTINFO, &val, sizeof (val))) < 0)
    zlog_warn ("DR4409:Can't set IP_PKTINFO option for fd %d to %d: %s",
	       sock,val,safe_strerror(errno));
#elif defined (IP_RECVIF)
  if ((ret = setsockopt (sock, IPPROTO_IP, IP_RECVIF, &val, sizeof (val))) < 0)
    zlog_warn ("DR4410:Can't set IP_RECVIF option for fd %d to %d: %s",
	       sock,val,safe_strerror(errno));
#else
...
setsockopt_ipv4_tos(int sock, int tos)
{
  ...
  if (ret < 0)
    zlog_warn ("DR4411:Can't set IP_TOS option for fd %d to %#x: %s",
...
#endif
      default:
        zlog_warn ("DR4412:setsockopt_ifindex: unknown address family %d", af);
    }
  return ret;
}
...
#endif
      default:
        zlog_warn ("DR4413:getsockopt_ifindex: unknown address family %d", af);
        return 0;
    }
}
...
#define TCP_MD5_AUTH_DEL 2
  ...
  if ((ret = setsockopt (sock, IPPROTO_TCP, TCP_MD5SIG, &md5sig, sizeof md5sig)) < 0)
	...
	zlog_err ("DR7453:sockopt_tcp_signature: setsockopt(%d): %s",
