...
if_create (const char *name, int namelen)
{
  ...
  if (if_lookup_by_name(ifp->name) == NULL)
    ...
    zlog_err("DR7150:if_create(%s): corruption detected -- interface with this "
...
if_dump (const struct interface *ifp)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (ifp->connected, node, c))
    zlog_info ("DR1000:Interface %s index %d metric %d mtu %d "
#ifdef HAVE_IPV6
               "mtu6 %d "
#endif /* HAVE_IPV6 */
               "%s",
...
connected_log (struct connected *connected, char *str)
{
  ...
  zlog (NULL, LOG_INFO, "DR1001:%s", logbuf);
}
  ...
  if (rn)
    {
      ...
      zlog_info ("DR1002:ifaddr_ipv4_add(): address %s is already added",
		 inet_ntoa (*ifaddr));
      return;
    }
  rn->info = ifp;
...
ifaddr_ipv4_delete (struct in_addr *ifaddr, struct interface *ifp)
{
  ...
  if (! rn)
    {
      zlog_info ("DR1003:ifaddr_ipv4_delete(): can't find address %s",
		 inet_ntoa (*ifaddr));
      return;
    }
  rn->info = NULL;
