...
smux_socket (void)
{
  ...
  if (gai)
    {
      zlog_warn("DR-4350:Cannot locate loopback service smux");
      return -1;
    }
  ...
  freeaddrinfo(res0);
  if (sock < 0)
    zlog_warn ("DR-4351:Can't connect to SNMP agent with SMUX");
#else
  sock = socket (AF_INET, SOCK_STREAM, 0);
  if (sock < 0)
    {
      zlog_warn ("DR-4352:Can't make socket for SNMP");
      return -1;
    }
...
smux_parse_close (u_char *ptr, int len)
{
  ...
  zlog_info ("DR-1050:SMUX_CLOSE with reason: %ld", reason);
}
...
process_rest: /* see note below: YYY */
  ...
  switch (type)
    ...
    case SMUX_OPEN:
      ...
      zlog_warn ("DR-4354:SMUX_OPEN received: resetting connection.");
      return -1;
      break;
    case SMUX_RREQ:
      /* SMUX_RREQ message is invalid for us. */
      zlog_warn ("DR-4355:SMUX_RREQ received: resetting connection.");
      return -1;
      break;
    case SMUX_SOUT:
      ...
      if (sout_save_len > 0)
        ...
        zlog_warn ("DR-4356:SMUX_SOUT sout_save_len=%d - invalid", (int) sout_save_len);
    ...
    case SMUX_GETRSP:
      ...
      zlog_warn ("DR-4357:SMUX_GETRSP received: resetting connection.");
      return -1;
      break;
    case SMUX_CLOSE:
	...
	zlog_debug ("SMUX_SET");
      ...
      break;
    default:
      zlog_info ("DR-1051:Unknown type: %d", type);
      break;
    }
  return 0;
}
...
smux_read (struct thread *t)
{
  ...
  if (len < 0)
    {
      zlog_warn ("DR-4358:Can't read all SMUX packet: %s", safe_strerror (errno));
      close (sock);
      smux_sock = -1;
  ...
  if (len == 0)
    {
      zlog_warn ("DR-4359:SMUX connection closed: %d", sock);
      close (sock);
      smux_sock = -1;
      smux_event (SMUX_CONNECT, 0);
      return -1;
      ...
      if (debug_smux) 
        {
          ...
          zlog_info ("DR-1052:BUFSIZ: %d // oid_len: %lu", BUFSIZ, (u_long)oid_len);
      }
...
smux_connect (struct thread *t)
{
  ...
  if (ret < 0)
    {
      zlog_warn ("DR-4360:SMUX open message send failed: %s", safe_strerror (errno));
      close (smux_sock);
      smux_sock = -1;
      if (++fail < SMUX_MAX_FAILURE)
	smux_event (SMUX_CONNECT, 0);
  ...
  if (ret < 0)
    {
      zlog_warn ("DR-4361:SMUX register message send failed: %s", safe_strerror (errno));
      close (smux_sock);
      smux_sock = -1;
      if (++fail < SMUX_MAX_FAILURE)
	smux_event (SMUX_CONNECT, 0);
