...
zlog_backtrace(int priority)
{
  ...
  if (size <= 0 || (size_t)size > array_size(array))
    {
      zlog_err("DR7200:Cannot get backtrace, returned invalid # of frames %d "
  ...
  if (!(strings = backtrace_symbols(array, size)))
    {
      zlog_err("DR7201:Cannot get backtrace symbols (out of memory?)");
      for (i = 0; i < size; i++)
  ...
  if (zl->filename)
    {
      ...
      if (zl->fp == NULL)
        {
	  zlog_err("DR7202:Log rotate failed: cannot open file %s for append: %s",
  ...
  const char *none, const char *mesname)
{
  ...
  zlog_err("DR7203:message index %d not found in %s (max is %d)", index, mesname, max);
  assert (none);
  return none;
}
...
zroute_lookup(u_int zroute)
{
  ...
  if (zroute >= array_size(route_types))
    {
      zlog_err("DR7204:unknown zebra route type: %u", zroute);
      return &unknown;
    }
  ...
  for (i = 0; i < array_size(route_types); i++)
    {
      if (zroute == route_types[i].type)
        {
	  zlog_warn("DR4050:internal error: route type table out of order "
		    "while searching for %u, please notify developers", zroute);
	  return &route_types[i];
        }
    }
  zlog_err("DR7205:internal error: cannot find route type %u in table!", zroute);
  return &unknown;
}
...
zserv_command_string (unsigned int command)
{
  if (command >= array_size(command_types))
    {
      zlog_err ("DR7206:unknown zserv command type: %u", command);
