...
zclient_flush_data(struct thread *thread)
{
  ...
  switch (buffer_flush_available(zclient->wb, zclient->sock))
    {
    case BUFFER_ERROR:
      zlog_warn("DR-4650:%s: buffer_flush_available failed on zclient fd %d, closing",
...
zclient_send_message(struct zclient *zclient)
{
  ...
  switch (buffer_write(zclient->wb, zclient->sock, STREAM_DATA(zclient->obuf),
    ...
    {
    case BUFFER_ERROR:
      zlog_warn("DR-4651:%s: buffer_write failed to zclient fd %d, closing",
      		 __func__, zclient->sock);
      return zclient_failed(zclient);
      break;
    case BUFFER_EMPTY:
...
zclient_start (struct zclient *zclient)
{
  ...
  if (set_nonblocking(zclient->sock) < 0)
    zlog_warn("DR-4652:%s: set_nonblocking(%d) failed", __func__, zclient->sock);
...
zebra_interface_bfd_read (struct stream *s, struct prefix *p)
{
  ...
  if (ifp == NULL)
    {
      zlog_warn ("DR-4653:zebra_interface_bfd_read: "
...
zebra_interface_address_read (int type, struct stream *s)
{
  ...
  if (ifp == NULL)
    {
      zlog_warn ("DR-4654:zebra_interface_address_read(%s): "
       ...
       if (ifc != NULL)
	 {
	   ...
	   if (ifc->destination)
	   ...
	   else if (CHECK_FLAG(ifc->flags, ZEBRA_IFA_PEER))
	     {
	       ...
	       zlog_warn("DR-4655:warning: interface %s address %s "
...
zclient_read (struct thread *thread)
{
  ...
  if (length < ZEBRA_HEADER_SIZE) 
    {
      zlog_err("DR-7601:%s: socket %d message length %u is less than %d ",
  ...
  if (length > STREAM_SIZE(zclient->ibuf))
    {
      struct stream *ns;
      zlog_warn("DR-4656:%s: message size %u exceeds buffer size %lu, expanding...",
...
zclient_serv_path_set (char *path)
{
  ...
  if (stat(path, &sb) == -1)
    {
      zlog_warn ("DR-4657:%s: zebra socket `%s' does not exist", __func__, path);
      return;
    }

  if ((sb.st_mode & S_IFMT) != S_IFSOCK)
    {
      zlog_warn ("DR-4658:%s: `%s' is not unix socket, sir", __func__, path);
      return;
    }
