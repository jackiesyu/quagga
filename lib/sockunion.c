...
sockunion_socket (union sockunion *su)
{
  ...
  if (sock < 0)
    {
      zlog (NULL, LOG_WARNING, "DR-4450:Can't make socket : %s", safe_strerror (errno));
      return -1;
    }
		   ...
		   unsigned int ifindex)
{
  ...
  if (ret < 0)
      ...
      if (errno != EINPROGRESS)
	{
	  ...
	  zlog_info ("DR-1150:can't connect to %s fd %d : %s",
...
sockunion_stream_socket (union sockunion *su)
{
  ...
  if (sock < 0)
    zlog (NULL, LOG_WARNING, "DR-4451:can't make socket sockunion_stream_socket");
		...
		union sockunion *su_addr)
{
  ...
  if (ret < 0)
    zlog (NULL, LOG_WARNING, "DR-4452:can't bind socket : %s", safe_strerror (errno));
...
sockopt_reuseaddr (int sock)
{
  ...
  if (ret < 0)
    {
      zlog (NULL, LOG_WARNING, "DR-4453:can't set sockopt SO_REUSEADDR to socket %d", sock);
      return -1;
    }
  return 0;
}
...
sockopt_reuseport (int sock)
{
  ...
  if (ret < 0)
    {
      zlog (NULL, LOG_WARNING, "DR-4454:can't set sockopt SO_REUSEPORT to socket %d", sock);
      return -1;
    }
  return 0;
}
  ...
  if (family == AF_INET)
    {
      ...
      if (ret < 0)
	{
	  zlog (NULL, LOG_WARNING, "DR-4455:can't set sockopt IP_TTL %d to socket %d", ttl, sock);
	  return -1;
	}
      return 0;
    }
  ...
  if (family == AF_INET6)
    {
      ...
      if (ret < 0)
	{
	  zlog (NULL, LOG_WARNING, "DR-4456:can't set sockopt IPV6_UNICAST_HOPS %d to socket %d",
		    ttl, sock);
	  return -1;
	}
      return 0;
  ...
  if (family == AF_INET)
    {
	  ...
	  zlog (NULL, LOG_WARNING,
		"DR-4457:can't set sockopt IP_MINTTL to %d on socket %d: %s",
  ...
  if (family == AF_INET6)
    {
      ...
      if (ret < 0)
	  zlog (NULL, LOG_WARNING,
		"DR-4458:can't set sockopt IPV6_MINHOPCNT to %d on socket %d: %s",
		minttl, sock, safe_strerror (errno));
      return ret;
    }
#endif
  ...
  if (family == AF_INET6)
    {
      ...
      if (ret < 0)
	{
	  zlog (NULL, LOG_WARNING, "DR-4459:can't set sockopt IPV6_V6ONLY "
		    "to socket %d", sock);
	  return -1;
	}
      return 0;
...
sockunion_getsockname (int fd)
{
  ...
  if (ret < 0)
    {
      zlog_warn ("DR-4460:Can't get local address and port by getsockname: %s",
		 safe_strerror (errno));
      return NULL;
    }
...
sockunion_getpeername (int fd)
{
  ...
  if (ret < 0)
    {
      zlog (NULL, LOG_WARNING, "DR-4461:Can't get remote address and port: %s",
	    safe_strerror (errno));
      return NULL;
    }
