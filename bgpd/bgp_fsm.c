...
bgp_stop (struct peer *peer)
{
  ...
  if (peer->status == Established)
	...
	zlog_info ("BGP1100:%%ADJCHANGE: neighbor %s Down %s", peer->host,
...
bgp_stop_with_notify (struct peer *peer, u_char code, u_char sub_code)
{
  ...
  if (CHECK_FLAG (peer->sflags, PEER_STATUS_ACCEPT_PEER))
    {
      zlog_info ("BGP1101:%s [Event] Accepting BGP peer is deleted", peer->host);
      peer_delete (peer);
      return -1;
    }
...
bgp_connect_success (struct peer *peer)
{
  if (peer->fd < 0)
    {
      zlog_err ("BGP7050:bgp_connect_success peer's fd is negative value %d",
		peer->fd);
      return -1;
    }
...
bgp_start (struct peer *peer)
{
  ...
  switch (status)
	...
	plog_debug (peer->log, "%s [FSM] Non blocking connect waiting result",
      ...
      if (peer->fd < 0)
	{
	  zlog_err ("BGP7051:bgp_start peer's fd is negative value %d",
		    peer->fd);
	  return -1;
	}
      BGP_READ_ON (peer->t_read, bgp_read, peer->fd);
...
bgp_establish (struct peer *peer)
{
  ...
  if (bgp_flag_check (peer->bgp, BGP_FLAG_LOG_NEIGHBOR_CHANGES))
    zlog_info ("BGP1102:%%ADJCHANGE: neighbor %s Up", peer->host);
