...
zlookup_read (void)
{
  ...
  if (version != ZSERV_VERSION || marker != ZEBRA_HEADER_MARKER)
    {
      zlog_err("BGP7250:%s: socket %d version mismatch, marker %d, version %d",
...
zlookup_query (struct in_addr addr)
{
  ...
  if (ret < 0)
    {
      zlog_err ("BGP7251:can't write to zlookup->sock");
      close (zlookup->sock);
      zlookup->sock = -1;
      return NULL;
    }
  if (ret == 0)
    {
      zlog_err ("BGP7252:zlookup->sock connection closed");
      close (zlookup->sock);
      zlookup->sock = -1;
      return NULL;
    }
