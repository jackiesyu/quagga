...
sighup (void)
{
  zlog (NULL, LOG_INFO, "BGP-1150:SIGHUP received");

  /* Terminate all thread. */
  bgp_terminate ();
  bgp_reset ();
  zlog_info ("BGP-1151:bgpd restarting!");
...
sigint (void)
{
  zlog_notice ("BGP-1152:Terminating on signal");
  ...
  for (ALL_LIST_ELEMENTS_RO(bm->listen_sockets, node, socket))
    {
      if (close ((int)(long)socket) == -1)
        zlog_err ("BGP-7100:close (%d): %s", (int)(long)socket, safe_strerror (errno));
    }
...
main (int argc, char **argv)
{
  ...
  if (daemon_mode && daemon (0, 0) < 0)
    {
      zlog_err("BGP-7101:BGPd daemon failed: %s", strerror(errno));
      return (1);
    }
  ...
  /* Print banner. */
  zlog_notice ("BGP-1153:BGPd %s starting: vty@%d, bgp@%s:%d", QUAGGA_VERSION,
	       vty_port, 
