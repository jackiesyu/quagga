...
bgp_connect_check (struct peer *peer)
{
  ...
  if (ret < 0)
    {
      zlog (peer->log, LOG_INFO, "BGP-1350:can't get sockopt for nonblocking connect");
  ...
  if (sub_code != BGP_NOTIFY_CEASE_CONFIG_CHANGE)
    {
      if (sub_code == BGP_NOTIFY_CEASE_ADMIN_RESET)
      {
        ...
        zlog_info ("BGP-1351:Notification sent to neighbor %s: User reset", peer->host);
      }
      else if (sub_code == BGP_NOTIFY_CEASE_ADMIN_SHUTDOWN)
      {
        peer->last_reset = PEER_DOWN_USER_SHUTDOWN;
        zlog_info ("BGP-1352:Notification sent to neighbor %s: shutdown", peer->host);
      }
      else
      {
        peer->last_reset = PEER_DOWN_NOTIFY_SEND;
        zlog_info ("BGP-1353:Notification sent to neighbor %s: type %u/%u",
                   peer->host, code, sub_code);
      }
    }
  else
     zlog_info ("BGP-1354:Notification sent to neighbor %s: configuration change",
                peer->host);
...
bgp_open_receive (struct peer *peer, bgp_size_t size)
{
  ...
  if (CHECK_FLAG (peer->cap, PEER_CAP_AS4_RCV) && !as4)
    {
      zlog_err ("BGP-7350:%s bad OPEN, got AS4 capability, but AS4 set to 0",
                peer->host);
  ...
  if (remote_as == BGP_AS_TRANS)
    {
      ...
      if (as4 == BGP_AS_TRANS)
        {
          zlog_err ("BGP-7351:%s [AS4] NEW speaker using AS_TRANS for AS4, not allowed",
                    peer->host);
          bgp_notify_send (peer, BGP_NOTIFY_OPEN_ERR,
                 BGP_NOTIFY_OPEN_BAD_PEER_AS);
          return -1;
  ...
  else 
    {
	  ...
	  zlog_err ("BGP-7352:%s bad OPEN, got AS4 capability, but remote_as %u"
  ...
  if (CHECK_FLAG (peer->sflags, PEER_STATUS_ACCEPT_PEER))
	...
	zlog_debug ("%s [Event] Transfer accept BGP peer to real (state %s)",
      ...
      if (peer->fd < 0)
	{
	  zlog_err ("BGP-7353:bgp_open_receive peer's fd is negative value %d",
		    peer->fd);
	  return -1;
	}
...
bgp_update_receive (struct peer *peer, bgp_size_t size)
{
  ...
  if (peer->status != Established) 
    {
      zlog_err ("BGP-7354:%s [FSM] Update packet received under status %s",
  ...
  if (stream_pnt (s) + 2 > end)
    {
      zlog_err ("BGP-7355:%s [Error] Update packet error"
  ...
  if (stream_pnt (s) + withdraw_len > end)
    {
      zlog_err ("BGP-7356:%s [Error] Update packet error"
  ...
  if (stream_pnt (s) + 2 > end)
    {
      zlog_warn ("BGP-4250:%s [Error] Packet Error"
  ...
  if (stream_pnt (s) + attribute_len > end)
    {
      zlog_warn ("BGP-4251:%s [Error] Packet Error"
      ...
      || BGP_DEBUG (update, UPDATE_IN))
    {
      ...
      if (attr_parse_ret == BGP_ATTR_PARSE_WITHDRAW)
        zlog (peer->log, LOG_ERR,
              "BGP-7357:%s rcvd UPDATE with errors in attr(s)!! Withdrawing route.",
              peer->host);
  ...
  if (size != BGP_MSG_ROUTE_REFRESH_MIN_SIZE - BGP_HEADER_SIZE)
    {
      ...
      if (size - (BGP_MSG_ROUTE_REFRESH_MIN_SIZE - BGP_HEADER_SIZE) < 5)
        {
          zlog_info ("BGP-1355:%s ORF route refresh length error", peer->host);
...
bgp_capability_msg_parse (struct peer *peer, u_char *pnt, bgp_size_t length)
{
  ...
  while (pnt < end)
    {      
      ...
      if (pnt + 3 > end)
        {
          zlog_info ("BGP-1356:%s Capability length error", peer->host);
          bgp_notify_send (peer, BGP_NOTIFY_CEASE, 0);
          return -1;
        }
      action = *pnt;
	  ...
	  && action != CAPABILITY_ACTION_UNSET)
        {
          zlog_info ("BGP-1357:%s Capability Action Value error %d",
		     peer->host, action);
          bgp_notify_send (peer, BGP_NOTIFY_CEASE, 0);
          return -1;
        }
...
bgp_read (struct thread *thread)
{
  ...
  if (peer->status == Connect)
  ...
  else
    {
      if (peer->fd < 0)
	{
	  zlog_err ("BGP-7358:bgp_read peer's fd is negative value %d", peer->fd);
	  return -1;
	}
      BGP_READ_ON (peer->t_read, bgp_read, peer->fd);
    }
