...
bgp_md5_set_socket (int socket, union sockunion *su, const char *password)
{
  ...
  if (ret < 0)
    zlog (NULL, LOG_WARNING, "BGP4150:can't set TCP_MD5SIG option on socket %d: %s",
...
bgp_md5_set_connect (int socket, union sockunion *su, const char *password)
{
  ...
  if ( bgpd_privs.change (ZPRIVS_RAISE) )
    {
      zlog_err ("BGP7200:%s: could not raise privs", __func__);
      return ret;
    }
  ...
  if (bgpd_privs.change (ZPRIVS_LOWER) )
    zlog_err ("BGP7201:%s: could not lower privs", __func__);
#endif /* HAVE_TCP_MD5SIG */
...
bgp_update_sock_send_buffer_size (int fd)
{
  ...
  if (getsockopt(fd, SOL_SOCKET, SO_SNDBUF, &optval, &optlen) < 0)
    {
      zlog_err("BGP7204:getsockopt of SO_SNDBUF failed %s\n", safe_strerror(errno));
      return;
    }
  if (optval < size)
    {
      if (setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &size, sizeof(size)) < 0)
        {
          zlog_err("BGP7205:Couldn't increase send buffer: %s\n", safe_strerror(errno));
        }
    }
}
  ...
  if (!peer->gtsm_hops && (peer_sort (peer) == BGP_PEER_EBGP))
    {
      ...
      if (ret)
	{
	  zlog_err ("BGP7206:%s: Can't set TxTTL on peer (rtrid %s) socket, err = %d",
		    __func__,
      ...
      if (ret)
	{
      ...
      if (ret)
	{
	  zlog_err ("BGP7208:%s: Can't set MinTTL on peer (rtrid %s) socket, err = %d",
		    __func__,
		    inet_ntop (AF_INET, &peer->remote_id, buf, sizeof(buf)),
		    errno);
	}
...
bgp_accept (struct thread *thread)
{
  ...
  if (accept_sock < 0)
    {
      zlog_err ("BGP7209:accept_sock is nevative value %d", accept_sock);
      return -1;
    }
  ...
  if (bgp_sock < 0)
    {
      zlog_err ("BGP7210:[Error] BGP socket accept failed (%s)", safe_strerror (errno));
      return -1;
    }
...
bgp_bind (struct peer *peer)
{
  ...
  if ( bgpd_privs.change (ZPRIVS_RAISE) )
  	zlog_err ("BGP7211:bgp_bind: could not raise privs");
  
  ret = setsockopt (peer->fd, SOL_SOCKET, SO_BINDTODEVICE, 
		    &ifreq, sizeof (ifreq));

  if (bgpd_privs.change (ZPRIVS_LOWER) )
    zlog_err ("BGP7212:bgp_bind: could not lower privs");

  if (ret < 0)
    {
      zlog (peer->log, LOG_INFO, "BGP1250:bind to interface %s failed", peer->ifname);
      return ret;
    }
...
#  endif
  ...
  if (ret < 0)
    {
      zlog_err ("BGP7217:bind: %s", safe_strerror (en));
      return ret;
    }
  ...
  if (ret < 0)
    {
      zlog_err ("BGP7218:listen: %s", safe_strerror (errno));
      return ret;
    }
...
bgp_socket (unsigned short port, const char *address)
{
  ...
  if (ret != 0)
    {
      zlog_err ("BGP7219:getaddrinfo: %s", gai_strerror (ret));
      return -1;
    }
  ...
  for (ainfo = ainfo_save; ainfo; ainfo = ainfo->ai_next)
	...
	continue;
      ...
      if (sock < 0)
	{
	  zlog_err ("BGP7220:socket: %s", safe_strerror (errno));
	  continue;
	}
  ...
  if (count == 0)
    {
      zlog_err ("BGP7221:%s: no usable addresses", __func__);
      return -1;
    }
...
bgp_socket (unsigned short port, const char *address)
{
  ...
  if (address && ((ret = inet_aton(address, &sin.sin_addr)) < 1))
    {
      zlog_err("BGP7223:bgp_socket: could not parse ip address %s: %s",
