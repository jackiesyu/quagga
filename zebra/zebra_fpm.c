...
zfpm_get_time (void)
{
  ...
  if (quagga_gettime (QUAGGA_CLK_MONOTONIC, &tv) < 0)
    zlog_warn ("DR5550:FPM: quagga_gettime failed!!");
...
zfpm_connection_down (const char *detail)
{
  ...
  zlog_info ("DR1700:connection to the FPM has gone down: %s", detail);
...
zfpm_connect_cb (struct thread *t)
{
  ...
  zlog_info ("DR1701:can't connect to FPM %d: %s", sock, safe_strerror (errno));
  close (sock);
