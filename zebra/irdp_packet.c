		  ...
		  struct interface *ifp)
{
  ...
  if (len != iplen)
    {
      zlog_err ("DR-8100:IRDP: RX length doesnt match IP length");
      return;
    }

  if (iplen < ICMP_MINLEN) 
    {
      zlog_err ("DR-8101:IRDP: RX ICMP packet too short from %s\n",
  	      inet_ntoa (src));
      return;
    }
  ...
  if (iplen > IRDP_RX_BUF-34) 
    {
      zlog_err ("DR-8102:IRDP: RX ICMP packet too long from %s\n",
	        inet_ntoa (src));
      return;
    }
  ...
  if (in_cksum (icmp, datalen) != icmp->checksum) 
    {
      zlog_warn ("DR-5200:IRDP: RX ICMP packet from %s. Bad checksum, silently ignored",
                 inet_ntoa (src));
      return;
    }
  ...
  if (icmp->code != 0) 
    {
      zlog_warn ("DR-5201:IRDP: RX packet type %d from %s. Bad ICMP type code,"
                 " silently ignored",
  ...
  if (! ((ntohl (ip->ip_dst.s_addr) == INADDR_BROADCAST)
        ...
        (ntohl (ip->ip_dst.s_addr) == INADDR_ALLRTRS_GROUP
         && !(irdp->flags & IF_BROADCAST)))
    {
      zlog_warn ("DR-5202:IRDP: RX illegal from %s to %s while %s operates in %s\n",
                 inet_ntoa (src),
                 ntohl (ip->ip_dst.s_addr) == INADDR_ALLRTRS_GROUP ?
      ...
      zlog_warn ("DR-5203:IRDP: Please correct settings\n");
      return;
    }
  ...
  switch (icmp->type) 
    ...
    default:
      zlog_warn ("DR-5204:IRDP: RX type %d from %s. Bad ICMP type, silently ignored",
		 icmp->type,
		 inet_ntoa (src));
    }
}
...
irdp_recvmsg (int sock, u_char *buf, int size, int *ifindex)
{
  ...
  if (ret < 0) {
    zlog_warn("DR-5205:IRDP: recvmsg: read error %s", safe_strerror(errno));
    return ret;
  }

  if (msg.msg_flags & MSG_TRUNC) {
    zlog_warn("DR-5206:IRDP: recvmsg: truncated message");
    return ret;
  }
  if (msg.msg_flags & MSG_CTRUNC) {
    zlog_warn("DR-5207:IRDP: recvmsg: truncated control message");
    return ret;
  }
	    ...
	    u_int32_t ttl)
{
  ...
  if (setsockopt(irdp_sock, IPPROTO_IP, IP_HDRINCL,
		 (char *) &on, sizeof(on)) < 0)
    zlog_warn("DR-5208:sendto %s", safe_strerror (errno));
