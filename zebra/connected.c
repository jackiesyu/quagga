  ...
  if (broad)
    {
	  ...
	  if (IPV4_ADDR_SAME(addr,broad))
	    zlog_warn("DR4700:warning: interface %s has same local and peer "
  ...
  else
    {
      if (CHECK_FLAG(ifc->flags, ZEBRA_IFA_PEER))
        {
	  zlog_warn("DR4701:warning: %s called for interface %s "
	...
	zlog_warn("DR4702:warning: PtP interface %s with addr %s/%d needs a "
		    ...
		    const char *label)
{
  ...
  if (broad)
	...
	zlog_warn("DR4703:warning: %s called for interface %s with unspecified "
  ...
  if (CHECK_FLAG(ifc->flags, ZEBRA_IFA_PEER) && !ifc->destination)
    {
      zlog_warn("DR4704:warning: %s called for interface %s "
		"with peer flag set, but no peer address supplied",
		__func__, ifp->name);
      UNSET_FLAG(ifc->flags, ZEBRA_IFA_PEER);
    }
