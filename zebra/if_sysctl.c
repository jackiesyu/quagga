...
#define MIBSIZ 6
  ...
  if (sysctl (mib, MIBSIZ, NULL, &bufsiz, NULL, 0) < 0) 
    {
      zlog_warn ("DR4850:sysctl() error by %s", safe_strerror (errno));
      return;
    }
  ...
  if (sysctl (mib, MIBSIZ, buf, &bufsiz, NULL, 0) < 0) 
    {
      zlog (NULL, LOG_WARNING, "DR4851:sysctl error by %s", safe_strerror (errno));
      return;
    }
...
#define MIBSIZ 6
  ...
  if (sysctl (mib, MIBSIZ, NULL, &bufsiz, NULL, 0) < 0) 
    {
      zlog (NULL, LOG_WARNING, "DR4850:sysctl() error by %s", safe_strerror (errno));
      return;
    }
  ...
  for (end = buf + bufsiz; buf < end; buf += ifm->ifm_msglen) 
	...
	case RTM_NEWADDR:
	  ...
	  break;
	default:
	  zlog_info ("DR1250:interfaces_list(): unexpected message type");
	  XFREE (MTYPE_TMP, ref);
	  return;
	  break;
	}
