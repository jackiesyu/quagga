	  ...
	  int add_leave)
{
  ...
  if(!p) {
        zlog_warn ("DR-5100:IRDP: can't get address for %s", ifp->name);
	return 1;
  }
  ...
  if (ret < 0)
    zlog_warn ("DR-5101:IRDP: %s can't setsockopt %s: %s",
...
irdp_if_start(struct interface *ifp, int multicast, int set_defaults)
{
  ...
  if (irdp->flags & IF_ACTIVE ) {
    zlog_warn("DR-5102:IRDP: Interface is already active %s", ifp->name);
    return;
  }
  if ((irdp_sock < 0) && ((irdp_sock = irdp_sock_init()) < 0)) {
    zlog_warn("DR-5103:IRDP: Cannot activate interface %s (cannot create "
  ...
  if (! (ifp->flags & IFF_UP)) {
    zlog_warn("DR-5104:IRDP: Interface is down %s", ifp->name);
  }
  ...
  if( multicast) {
    ...
    if (! (ifp->flags & (IFF_MULTICAST|IFF_ALLMULTI))) {
      zlog_warn("DR-5105:IRDP: Interface not multicast enabled %s", ifp->name);
    }
  }
...
irdp_if_stop(struct interface *ifp)
{
  ...
  if (irdp == NULL) {
    zlog_warn ("DR-5106:Interface %s structure is NULL", ifp->name);
    return;
  }

  if (! (irdp->flags & IF_ACTIVE )) {
    zlog_warn("DR-5107:Interface is not active %s", ifp->name);
    return;
  }
...
irdp_if_shutdown(struct interface *ifp)
{
  ...
  if (irdp->flags & IF_SHUTDOWN ) {
    zlog_warn("DR-5108:IRDP: Interface is already shutdown %s", ifp->name);
    return;
  }
...
irdp_if_no_shutdown(struct interface *ifp)
{
  ...
  if (! (irdp->flags & IF_SHUTDOWN )) {
    zlog_warn("DR-5109:IRDP: Interface is not shutdown %s", ifp->name);
    return;
  }
