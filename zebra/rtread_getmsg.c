...
route_read (void)
{
	...
	if ((dev = open (_PATH_GETMSG_ROUTE, O_RDWR)) == -1) {
		zlog_warn ("DR5450:can't open %s: %s", _PATH_GETMSG_ROUTE,
			safe_strerror (errno));
		return;
	}
	...
	if (putmsg (dev, &msgdata, NULL, flags) == -1) {
		zlog_warn ("DR5451:putmsg failed: %s", safe_strerror (errno));
		goto exit;
	}
	...
	while (1) {
		...
		if (retval == -1) {
			zlog_warn ("DR5452:getmsg(ctl) failed: %s", safe_strerror (errno));
			goto exit;
		}
		...
		if (msgdata.len >= sizeof (struct T_error_ack) &&
			...
			zlog_warn ("DR5453:getmsg(ctl) returned T_ERROR_ACK: %s",
		...
		if (retval != MOREDATA ||
			...
			TLIack->MGMT_flags != T_SUCCESS) {
			errno = ENOMSG;
			zlog_warn ("DR5454:getmsg(ctl) returned bizarreness");
			break;
		}
		...
		do {
			...
			if (retval == -1) {
				zlog_warn ("DR5455:getmsg(data) failed: %s",
					safe_strerror (errno));
				goto exit;
			}

			if (!(retval == 0 || retval == MOREDATA)) {
				zlog_warn ("DR5456:getmsg(data) returned %d", retval);
				goto exit;
			}
			...
			if (process) {
				if (msgdata.len %
					...
					zlog_warn ("DR5457:getmsg(data) returned "
