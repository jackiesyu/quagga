...
#define IFNUM_BASE 32
  ...
  if (sock < 0)
    {
      zlog_warn ("DR4800:Can't make %s socket stream: %s",
...
calculate_lifc_len:     /* must hold privileges to enter here */
  ...
  if (ret < 0)
    {
      zlog_warn ("DR4801:interface_list_ioctl: SIOCGLIFNUM failed %s",
  ...
  if (needed > lastneeded || needed < lastneeded / 2)
    {
      ...
      if ((buf = XMALLOC (MTYPE_TMP, needed)) == NULL)
        {
          zlog_warn ("DR4802:interface_list_ioctl: malloc failed");
          close (sock);
          return -1;
        }
    }
  ...
  if (ret < 0)
    {
      ...
      zlog_warn ("DR4803:SIOCGLIFCONF: %s", safe_strerror (errno));
...
if_get_index (struct interface *ifp)
{
  ...
  if (ret < 0)
    {
      zlog_warn ("DR4804:SIOCGLIFINDEX(%s) failed", ifp->name);
      return ret;
    }
  ...
  if (af == AF_INET)
      ...
      if (ret < 0)
        {
          if (errno != EADDRNOTAVAIL)
            {
              zlog_warn ("DR4805:SIOCGLIFNETMASK (%s) fail: %s", ifp->name,
                         safe_strerror (errno));
              return ret;
            }
          return 0;
      ...
      if (if_ioctl_ipv6 (SIOCGLIFSUBNET, (caddr_t) & lifreq) < 0)
	{
	  if (ifp->flags & IFF_POINTOPOINT)
	    ...
	    zlog_warn ("DR4806:SIOCGLIFSUBNET (%s) fail: %s",
