...
rib_queue_add (struct zebra_t *zebra, struct route_node *rn)
{
  ...
  if (zebra->ribq == NULL)
    {
      zlog_err ("DR-8400:%s: work_queue does not exist!", __func__);
      return;
    }
...
rib_queue_init (struct zebra_t *zebra)
{
  ...
  if (! (zebra->ribq = work_queue_new (zebra->master, 
    ...
    {
      zlog_err ("DR-8401:%s: could not initialise work queue!", __func__);
      return;
    }
  ...
  {
    zlog_err ("DR-8402:%s: could not initialise meta queue!", __func__);
    return;
  }
  return;
}
...
void rib_lookup_and_dump (struct prefix_ipv4 * p)
{
  ...
  if (! table)
  {
    zlog_err ("DR-8403:%s: vrf_table() returned NULL", __func__);
    return;
  }
