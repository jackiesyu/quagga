...
sighup (void)
{
  zlog_info ("DR-1450:SIGHUP received");
...
sigint (void)
{
  zlog_notice ("DR-1451:Terminating on signal");
...
main (int argc, char **argv)
{
  ...
  */
  ...
  if (daemon_mode && daemon (0, 0) < 0)
    {
      zlog_err("DR-8200:Zebra daemon failed: %s", strerror(errno));
      exit (1);
    }
  ...
  */
  ...
  /* Print banner. */
  zlog_notice ("DR-1452:Zebra %s starting: vty@%d", QUAGGA_VERSION, vty_port);
