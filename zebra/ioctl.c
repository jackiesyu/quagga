...
if_get_mtu (struct interface *ifp)
{
  ...
  if (if_ioctl (SIOCGIFMTU, (caddr_t) & ifreq) < 0) 
    {
      zlog_info ("DR1300:Can't lookup mtu by ioctl(SIOCGIFMTU)");
      ifp->mtu6 = ifp->mtu = -1;
      return;
    }
...
#else
  zlog (NULL, LOG_INFO, "DR1301:Can't lookup mtu on this system");
  ifp->mtu6 = ifp->mtu = -1;
#endif
}
...
if_get_flags (struct interface *ifp)
{
  ...
  if (ret < 0) 
    {
      zlog_err("DR7808:if_ioctl(SIOCGIFFLAGS) failed: %s", safe_strerror(errno));
      return;
    }
  ...
  if (CHECK_FLAG (ifp->status, ZEBRA_INTERFACE_LINKDETECTION))
    {
      ...
      if (if_ioctl(SIOCGIFMEDIA, (caddr_t) &ifmr) < 0)
        zlog_err("DR7809:if_ioctl(SIOCGIFMEDIA) failed: %s", safe_strerror(errno));
...
if_set_flags (struct interface *ifp, uint64_t flags)
{
  ...
  if (ret < 0)
    {
      zlog_info ("DR1302:can't set interface flags");
      return ret;
    }
  return 0;
}
