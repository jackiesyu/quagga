...
#define ND_BUFFER_SIZE 1024
  ...
  */
  if (cmd == ND_SET)
    ...
    zlog_err("DR7950:internal error - inappropriate command given to "
  ...
  if ( zserv_privs.change (ZPRIVS_RAISE) )
       zlog_err ("DR7951:solaris_nd: Can't raise privileges");
  if ((fd = open (device, O_RDWR)) < 0) 
    {
      zlog_warn("DR5000:failed to open device %s - %s", device, safe_strerror(errno));
      if ( zserv_privs.change (ZPRIVS_LOWER) )
        zlog_err ("DR7952:solaris_nd: Can't lower privileges");
      return -1;
    }
  if (ioctl (fd, I_STR, &strioctl) < 0) 
    {
      ...
      if ( zserv_privs.change (ZPRIVS_LOWER) )
        zlog_err ("DR7952:solaris_nd: Can't lower privileges");
      close (fd);
      zlog_warn("DR5001:ioctl I_STR failed on device %s - %s",
  ...
  if (cmd == ND_GET) 
    {
      ...
      if (errno) 
        {
          zlog_warn("DR5002:failed to convert returned value to integer - %s",
                    safe_strerror(errno));
          retval = -1;
        }
    } 
