...
zserv_flush_data(struct thread *thread)
{
  ...
  switch (buffer_flush_available(client->wb, client->sock))
    {
    case BUFFER_ERROR:
      zlog_warn("DR5600:%s: buffer_flush_available failed on zserv client fd %d, "
...
zebra_server_send_message(struct zserv *client)
{
  ...
  switch (buffer_write(client->wb, client->sock, STREAM_DATA(client->obuf),
    ...
    {
    case BUFFER_ERROR:
      zlog_warn("DR5601:%s: buffer_write failed to zserv client fd %d, closing",
...
zread_hello (struct zserv *client)
{
  ...
  &&  (proto > ZEBRA_ROUTE_STATIC))
    {
      zlog_notice ("DR1750:client %d says hello and bids fair to announce only %s routes",
                    client->sock, zebra_route_string(proto));

      /* if route-type was binded by other client */
      if (route_type_oaths[proto])
        zlog_warn ("DR5602:sender of %s routes changed %c->%c",
...
zebra_score_rib (int client_sock)
{
  ...
  for (i = ZEBRA_ROUTE_RIP; i < ZEBRA_ROUTE_MAX; i++)
    if (client_sock == route_type_oaths[i])
      {
        zlog_notice ("DR1751:client %d disconnected. %lu %s routes removed from the rib",
...
zebra_client_read (struct thread *thread)
{
  ...
  if (length < ZEBRA_HEADER_SIZE) 
    {
      zlog_warn("DR5603:%s: socket %d message length %u is less than header size %d",
  ...
  if (length > STREAM_SIZE(client->ibuf))
    {
      zlog_warn("DR5604:%s: socket %d message length %u exceeds buffer size %lu",
  ...
  switch (command) 
    ...
    case ZEBRA_HELLO:
      ...
      break;
    default:
      zlog_info ("DR1752:Zebra received unknown command %d", command);
      break;
    }
...
zebra_accept (struct thread *thread)
{
  ...
  if (client_sock < 0)
    {
      zlog_warn ("DR5605:Can't accept zebra socket: %s", safe_strerror (errno));
      return -1;
    }
...
zebra_serv ()
{
  ...
  if (accept_sock < 0) 
    {
      zlog_warn ("DR5606:Can't create zserv stream socket: %s", 
                 safe_strerror (errno));
      zlog_warn ("DR5607:zebra can't provice full functionality due to above error");
      return;
    }
  ...
  if (ret < 0)
    {
      zlog_warn ("DR5608:Can't bind to stream socket: %s", 
                 safe_strerror (errno));
      zlog_warn ("DR5607:zebra can't provice full functionality due to above error");
  ...
  if (ret < 0)
    {
      zlog_warn ("DR5610:Can't listen to stream socket: %s", 
                 safe_strerror (errno));
      zlog_warn ("DR5607:zebra can't provice full functionality due to above error");
      close (accept_sock);	/* Avoid sd leak. */
      return;
...
zebra_serv_un (const char *path)
{
  ...
  if (sock < 0)
    {
      zlog_warn ("DR5612:Can't create zserv unix socket: %s", 
                 safe_strerror (errno));
      zlog_warn ("DR5613:zebra can't provide full functionality due to above error");
      return;
    }
  ...
  if (ret < 0)
    {
      zlog_warn ("DR5614:Can't bind to unix socket %s: %s", 
  ...
  if (ret < 0)
    {
      zlog_warn ("DR5616:Can't listen to unix socket %s: %s", 
                 path, safe_strerror (errno));
      zlog_warn ("DR5613:zebra can't provide full functionality due to above error");
      close (sock);
      return;
